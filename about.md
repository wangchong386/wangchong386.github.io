...
* 责任描述：
    * 将Flume采集过来的snappy文件直接落到HDFS上，通过pig脚本解析到表入库，经过ods基础层-->mds中间层-->sds应用层的ETL处理，然后通过sqoop/sqlloader推送到RDBMS关系数据库中。ETL使用的hive sql/Pig计算+shell脚本来实现的。对整个ETL流程优化，保证数据实时准确完成处理。
    * 实时计算：将Flume采集的数据发送到kafka进行缓存，通过sparkstreaming+sparksql进行计算处理,对卖家店铺以及广告数据做实时统计。
二. 联通华盛经营分析系统
